...
docker inspect kenchrcum/ansible-playbook-operator:0.1.2 | jq -r '.[0].RepoDigests[0]'
...
curl -H "Accept: application/vnd.docker.distribution.manifest.v2+json" \
  https://registry-1.docker.io/v2/kenchrcum/ansible-playbook-operator/manifests/0.1.2 | \
  jq -r '.config.digest'
```
...
OPERATOR_DIGEST=$(crane digest kenchrcum/ansible-playbook-operator:0.1.2)
