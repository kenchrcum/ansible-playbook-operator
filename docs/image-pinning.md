...
operator:
  image:
    repository: kenchrcum/ansible-playbook-operator
    tag: "0.1.5"  # Ignored when digest is present
    digest: "sha256:1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef"
...
docker pull kenchrcum/ansible-playbook-operator:0.1.5
docker inspect kenchrcum/ansible-playbook-operator:0.1.5 | grep -i digest

# Or use docker manifest
docker manifest inspect kenchrcum/ansible-playbook-operator:0.1.5
```
...
# Get digest
skopeo inspect docker://kenchrcum/ansible-playbook-operator:0.1.5
```
...
# 1. Get current image
kubectl get pods -n ansible-operator-system -o jsonpath='{.items[0].spec.containers[0].image}'
# Output: kenchrcum/ansible-playbook-operator:0.1.5

# 2. Get digest for current image
CURRENT_DIGEST=$(crane digest kenchrcum/ansible-playbook-operator:0.1.5)
